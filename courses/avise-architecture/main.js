...
(function () {
  ...
  /* Nav dot tracking — IntersectionObserver-based.
     More reliable than scroll-math: works correctly for very tall modules,
     short modules at the bottom, and during fast scroll. The module whose
     center is closest to the viewport center is considered active. */
  let activeModuleIdx = -1;
  // Track which modules the learner has completed (quiz interacted with).
  // Persists to localStorage so progress survives a page reload.
  const completedModules = (() => {
    try { return new Set(JSON.parse(localStorage.getItem('course-completed') || '[]')); }
    catch (_) { return new Set(); }
  })();
  function saveCompleted() {
    try { localStorage.setItem('course-completed', JSON.stringify([...completedModules])); }
    catch (_) { /* localStorage may be disabled — fail quietly */ }
  }
  function markModuleCompleted(moduleId) {
    if (!moduleId) return;
    completedModules.add(moduleId);
    saveCompleted();
    // Find the corresponding nav-dot and add .completed
    const dot = navDots.find(d => d && d.dataset && d.dataset.target === moduleId);
    if (dot) dot.classList.add('completed');
  }
  // On load, restore completed marks
  navDots.forEach(dot => {
    if (dot && dot.dataset && completedModules.has(dot.dataset.target)) {
  ...
  function findActiveModule() {
    ...
    modules.forEach((mod, i) => {
      const modMid = mod.offsetTop + mod.offsetHeight / 2;
      const dist = Math.abs(modMid - viewportMid);
      if (dist < bestDist) {
        bestDist = dist;
        bestIdx = i;
      }
    });
    setActiveModule(bestIdx);
  }
  ...
  window.addEventListener('resize', findActiveModule, { passive: true });
  updateProgress();
  findActiveModule();
  ...
  window.checkQuiz = function (containerId) {
    ...
    if (!container) return;
    // Mark the parent module as completed (the quiz being checked = end of module)
    const parentModule = container.closest('.module');
    if (parentModule && parentModule.id) {
      markModuleCompleted(parentModule.id);
    }
  ...
  function initChat(containerEl) {
    ...
    function showPrev() {
      if (index <= 0) return;
      index--;
      // Hide the most recently shown message
      if (messages[index]) {
  ...
  function initFlow(containerEl) {
    ...
    const payloadEl  = $('.flow-payload',    containerEl);
    ...
    function updateProgress() {
      ...
      const prevBtn = $('.flow-prev-btn', containerEl);
      const nextBtn = $('.flow-next-btn', containerEl);
      // Prev disabled at the start
      if (prevBtn) prevBtn.disabled = (step <= 0);
      // When at the end: morph Next button into "Done" state
      if (nextBtn) {
        if (step >= stepsData.length && stepsData.length > 0) {
          // Reached the end — morph into a clear "Finished" state with success styling
          nextBtn.classList.add('flow-done');
          nextBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-0.25em;margin-right:.5em"><path d="M20 6 9 17l-5-5"/></svg>You finished — restart?';
          nextBtn.disabled = false;
          containerEl.classList.add('flow-completed');
        } else {
          nextBtn.classList.remove('flow-done');
          nextBtn.textContent = 'Next Step →';
          nextBtn.disabled = false;
          containerEl.classList.remove('flow-completed');
        }
      }
    }
    ...
    function animatePacket(fromId, toId) {
      if (!packet) return;
      // Scope lookup to this flow-animation so multiple flows on one page don't collide
    ...
    function applyState(s, animatePacketToo) {
      ...
      if (s && labelEl) labelEl.textContent = s.label || '';
      if (payloadEl) {
        if (s && s.payload) {
          payloadEl.textContent = s.payload;
          payloadEl.classList.add('visible');
        } else {
          payloadEl.textContent = '';
          payloadEl.classList.remove('visible');
        }
      }
      if (animatePacketToo && s && s.packet && s.from && s.to) {
        animatePacket('flow-' + s.from, 'flow-' + s.to);
      }
    }

    function next() {
      if (step >= stepsData.length) return;
      applyState(stepsData[step], true);
      step++;
      updateProgress();
    }

    function prev() {
      if (step <= 1) { reset(); return; }
      step--;
      // Re-apply the state of the most recently shown step (no packet replay)
      applyState(stepsData[step - 1], false);
      updateProgress();
    }
    ...
    function reset() {
      ...
      if (payloadEl) { payloadEl.textContent = ''; payloadEl.classList.remove('visible'); }
  ...
  /* ── CONDITIONAL FLOW / FEATURE FLAG MAP ─────────────────────
     Toggle between state-panels when state-btn is clicked.
     Each .state-btn[data-state="X"] activates .state-panel[data-state="X"]. */
  $$('.conditional-flow').forEach(container => {
    const btns = $$('.state-btn', container);
    const panels = $$('.state-panel', container);
    btns.forEach(btn => {
      btn.addEventListener('click', () => {
        const target = btn.dataset.state;
        btns.forEach(b => {
          const isActive = b.dataset.state === target;
          b.classList.toggle('active', isActive);
          b.setAttribute('aria-selected', isActive ? 'true' : 'false');
        });
        panels.forEach(p => {
          p.classList.toggle('active', p.dataset.state === target);
        });
      });
    });
  });
